...
class VoucherController extends Controller
{
    ...
    public function table(Voucher $model, User $user)
    {
        $vouchers = $model->newQuery()->where('created_by', $user->getParentAgency(auth()->id()));
    ...
    public function cashVoucher(Request $request, User $user, Information $information)
    {
        $results = Voucher::query()->where('id', $request->id)->with(['information'])->first();
        $agency = $information->where('user_id', $user->getParentAgency(auth()->id()))->first();
        return view('components.agency.voucher', compact('results', 'agency'));
    }

    public function export()
    {
        $date_now = Carbon::now()->format('Y-m-d_h:i:sA');
        return Excel::download(new VoucherExport, "voucers-{$date_now}.xlsx");
    }
}
