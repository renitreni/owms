<x-app-layout>
    ...
    <div id="app" class="pb-12 pt-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="pt-3 pl-3">
                    <button @click="cases_mdl= true"
                            class="bg-indigo-400 hover:bg-indigo-300 p-2 rounded shadow text-white mx-1">
                        <i class="fas fa-exclamation-triangle"></i> List Of Heinous Crimes
                    </button>
                    <button @click="notify_mdl= true"
                            class="bg-yellow-400 hover:bg-yellow-300 p-2 rounded shadow text-white mx-1">
                        <i class="fas fa-envelope-open"></i> Persons to Notify
                    </button>
                </div>
                <div class="p-3">
        ...
        <!--    Persons to Notify Modal-->
        <transition name="slide-fade">
            <div class="fixed inset-0 overflow-auto" v-if="notify_mdl">
                <div
                    class="flex items-end justify-center pt-20 px-4 pb-20 text-center sm:block sm:p-0  overflow-auto"
                >
                    <div class="fixed inset-0 transition-opacity" aria-hidden="true">
                                ...
                                <div class="flex-1 mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-4">
                                        <div class="flex flex-row">
                                            <div class="mx-1">
                                                <label>Email</label>
                                                <input v-model="ptn.email" type="email"
                                                       class="w-full border-0 bg-gray-100 rounded text-black outline-none focus:ring-opacity-0 p-2">
                                            </div>
                                            <div class="mx-1">
                                        ...
                                        <div>
                                            <table class="min-w-full divide-y divide-gray-200 mt-4">
                                                ...
                                                <tbody class="bg-white divide-y divide-gray-200">
                                                <tr v-for="item in email_list">
                                                    <td class="px-6 py-4 whitespace-nowrap">
                                                        @{{ item.email }}
                                                    </td>
                                                    <td class="px-0 py-0 whitespace-nowrap">
                                                        <button @click="deleteEmail(item.id)"
                                                                class="bg-red-400 hover:bg-red-300 p-2 rounded shadow text-white">
                                                            <i class="fas fa-trash"></i>
                                                        </button>
                                                    </td>
        ...
        <script>
            const e = new Vue({
                ...
                methods: {
                    ...
                    deleteEmail(id) {
                        var $this = this;
                        axios.post('{{ route('delete.ptn.emails') }}', {id: id}).then(function (value) {
                            $this.getEmails();
                        });
                    },
                    addEmail() {
                        if (this.ptn.email === '')
                            return false;
                        var $this = this;
                        axios.post('{{ route('store.ptn.email') }}', this.ptn).then(function (value) {
                            $this.ptn = {
                                email: '',
                            };
                            $this.getEmails();
                    ...
                    getEmails() {
                        var $this = this;
                        axios.post('{{ route('get.ptn.email') }}', this.heinous).then(function (value) {
